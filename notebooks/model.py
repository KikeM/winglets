...
def make_symmetric(point, axis = 'x'):
    ...
    """
    if axis == 'x':
        return point - Point([2.0 * point.x, 
                              0, 
                              0])
...
def winglet_tip_coordinates(length, sweep, cant):
    ...
    Point
    """
    return length * Point([np.sin(sweep) * np.sin(cant), 
                           np.cos(sweep) * np.sin(cant),
                           np.cos(cant)])

def generate_winglet(airfoil, location_le, length, chord, ratio_taper, sweep, cant, twist):
...
def generate_winglets(airfoil, location_le, length, chord, ratio_taper, sweep, cant, twist):
    """
    ...
    winglet_rhs = generate_winglet(airfoil      = airfoil, 
    ...
    winglet_lhs = generate_winglet(airfoil      = airfoil, 
